...
fn doctor(root: &std::path::Path, agent: Option<Agent>) -> Result<()> {
    ...
    match agent {
        ...
        Some(Agent::Opencode) => {
            println!(
                "opencode plugin scaffold target: {}",
                root.join(".opencode/plugins/code-sanity.ts").display()
            );
            println!("opencode can redirect read/edit tools via plugin hooks in a later phase");
        }
        None => {
...
"#,
        ...
        Agent::Opencode => {
            ...
            fs::create_dir_all(&dir).with_context(|| format!("create {}", dir.display()))?;
            fs::write(
                dir.join("code-sanity.ts"),
                "export default async function CodeSanityPlugin() {\n  return {\n    async 'tool.execute.before'(input: any, output: any) {\n      return output\n    }\n  }\n}\n",
            )
            .context("write opencode plugin scaffold")?;
        }
    }
    println!("installed scaffold for {installed}");
    Ok(())
}
